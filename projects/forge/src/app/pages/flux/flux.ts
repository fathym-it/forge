...
export class FluxComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    if (this.environment == "stg" || this.environment == "int") this.CanUnprovision = true;
    else this.CanUnprovision = false;
  ...
  public ngOnInit() {
    ...
    this.flowLayoutConfiguration.Init(
      this.FlowLayoutBehavior,
      this.FlowLayoutBehavior.NodeFactory,
      this.BeforeStartConnect,
      this.FlowLayoutBehavior.BeforeConnect,
      this.FlowLayoutBehavior.BeforeStartDetach,
      this.ToggleSelection,
      this.RemoveEdge,
      this.CanvasClick,
      this.FlowLayoutBehavior.ObjectRepainted,
      this.FlowLayoutBehavior.EdgeAdded,
      this.FlowLayoutBehavior.EdgeMoved,
  ...
  public HasErrors() {
    ...
    if (this.Flow) {
      this.Flow.Modules.forEach(function(module) {
        if (!module.Deleted && module.Settings && module.Settings.HasErrors) hasErrors = true;
      });
    }
    return hasErrors;
  }
  ...
  public Load(setMainLoader?: boolean) {
    ...
    //this.MainLoader = true;
    var self = this;
    forkJoin(this.flowSvc.Get(this.flowId), this.flowSvc.LoadModuleOptions(this.flowId)).subscribe(
      results => {
        if (isResultSuccess(results[0])) {
          this.Flow = results[0].Model;

          if (
            this.Flow.ProvisionStatus &&
            (this.Flow.ProvisionStatus == "Provisioned" || this.Flow.ProvisionStatus == "ProvisionedWithErrors")
          ) {
        ...
        if (isResultSuccess(results[1])) {
          ...
          var cats = this.ModuleOptions.map(m => m.Category)
          ...
          this.EditorModuleOptions = cats.map(c => {
            return {
              ...
              Visible: localStorage["Fathym:Flow:" + this.flowId + ":SideNav:" + c]
                ? localStorage["Fathym:Flow:" + this.flowId + ":SideNav:" + c] == "true"
                : true
            };
          });

          this.SideNavOpen = localStorage["Fathym:Flow:" + this.flowId + ":SideNavOpen"]
            ? localStorage["Fathym:Flow:" + this.flowId + ":SideNavOpen"] == "true"
            : true;
    ...
    this.SaveFlow(true).add(() => {
      ...
      this.flowSvc.Provision(this.Flow.ID, this.environment).subscribe(
        results => {
          ...
          if (!this.intervalRunning) this.setStatusCheckTimer(false);
        },
        err => {
          this.ModulesLoading = false;
          ...
          if (!this.intervalRunning) this.setStatusCheckTimer(true);
        },
        () => {
          this.ModulesLoading = false;
  ...
  public SaveFlow(forProvision?: boolean) {
    ...
    copy.Modules.forEach(function(module) {
      module.Token = null;
      if (forProvision || (module.Status && module.Status.Code != -100)) module.Status = null;
    });
  ...
  protected setStatusCheckTimer(stillProvisioning: boolean) {
    ...
    if (!this.intervalRunning && !this.statusChecking) {
      ...
      var result = self.flowSvc.GetFlowStatus(self.Flow.ID, this.environment).subscribe(
        result => {
          ...
          stillProvisioning =
            result.Model["ProvisionStatus"].Message != "Provisioned" &&
          ...
          if (self.CanUnprovision)
            self.ShowUnprovisioning =
              !stillProvisioning &&
              (result.Model["ProvisionStatus"].Message == "Provisioned" ||
                result.Model["ProvisionStatus"].Message == "ProvisionedWithErrors");
          else self.ShowUnprovisioning = false;

          if (result.Model["ProvisionStatus"].Message == "Unprovisioned") self.setUnprovisioningStatus();
          else {
            self.Flow.Modules.filter(function(item) {
              return !item.Deleted;
            }).forEach(function(module) {
              if (module.Status && module.Status.Code == -100) {
                ...
                if (modStatus) {
                  if (
                    (module.Status == null || modStatus.Code != module.Status.Code) &&
                    !(stillProvisioning && modStatus.Code != 0)
                  ) {
    ...
    setInterval(function() {
      if (!self.flowId) self.timerRunning = false;
              ...
              self.Flow.Modules.filter(function(item) {
                ...
                if (module.Status && module.Status.Code == -100) {
                  ...
                  if (modStatus) {
                    if (
                      module.Status == null ||
                      (modStatus.Code != module.Status.Code && !(stillProvisioning && modStatus.Code != 0))
                    ) {
                      module.Status = modStatus;
                      self.FlowLayoutBehavior.RefreshNode(module.ID);
                    }
